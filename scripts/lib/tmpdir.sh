...
#
# the earlier change: `rustdoc --test` (and, more generally, any `cc`/`ld`
...
# (concurrent worktree sessions). Once combined usage crosses roughly 13 GB,
...
# default below intentionally matches the exact path the earlier change
...
# abort the entire gate script over a diagnostic. review finding.
if command -v df >/dev/null 2>&1; then
  if fs_type_raw="$(df --output=fstype "$RAIKIRI_GATE_TMPDIR" 2>/dev/null)"; then
    ...
    if [[ "$fs_type" == "tmpfs" ]]; then
      ...
      echo "         This is the exact condition this temporary-directory setting avoids; set" >&2
