...
pub struct ElementData {
    ...
    /// Measured `line-break:anywhere` paragraph height for a synthetic inline root.
    /// `preshape_text` puts this size on the root and Taffy skips its DOM text
    /// children; paint still walks those children to preserve per-node styling.
    pub(crate) shared_inline_height: Option<f32>,
    /// Per-line background fragments for inline descendants in a shared layout.
    pub(crate) shared_inline_fragments: Option<Vec<InlineTextFragment>>,
}

/// One rectangle produced by shared inline text shaping for an inline box.
#[doc(hidden)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct InlineTextFragment {
    /// Horizontal offset from the shared paragraph's content origin.
    pub x: f32,
    /// Vertical offset from the shared paragraph's content origin.
    pub y: f32,
    /// Fragment width in CSS pixels.
    pub width: f32,
    /// Fragment height in CSS pixels.
    pub height: f32,
}
...
pub struct TextData {
    ...
    /// Shared paragraph layout for an inline text node participating in a
    /// `line-break:anywhere` synthetic inline root.
    pub(crate) shared_inline_text_layout: Option<Arc<parley::Layout<usize>>>,
...
impl Node {
    ...
    pub(crate) fn new_element(tag: SmolStr, style: Style, inline_style: Option<SmolStr>) -> Self {
        Self {
            ...
            data: NodeData::Element(Box::new(ElementData {
                ...
                shared_inline_height: None,
                shared_inline_fragments: None,
            })),
        }
    }
    ...
    pub(crate) fn new_text(text: SmolStr) -> Self {
        Self {
            ...
            data: NodeData::Text(TextData {
                text_content: text,
                text_layout: None,
                shared_inline_text_layout: None,
    ...
    /// Shared paragraph layout for inline text nodes, if present.
    #[doc(hidden)]
    #[inline]
    pub fn shared_inline_text_layout(&self) -> Option<&parley::Layout<usize>> {
        match &self.data {
            NodeData::Text(text) => text.shared_inline_text_layout.as_deref(),
            _ => None,
        }
    }

    /// Inline background fragments generated from a shared paragraph layout.
    #[doc(hidden)]
    #[inline]
    pub fn shared_inline_fragments(&self) -> Option<&[InlineTextFragment]> {
        match &self.data {
            NodeData::Element(element) => element.shared_inline_fragments.as_deref(),
            _ => None,
        }
    }

    /// Measured line height when this element is a shared inline formatting root.
    #[inline]
    pub(crate) fn shared_inline_height(&self) -> Option<f32> {
        match &self.data {
            NodeData::Element(element) => element.shared_inline_height,
            _ => None,
        }
    }
