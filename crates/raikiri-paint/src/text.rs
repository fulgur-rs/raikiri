...
fn is_autospace_inline_box(item: &PositionedLayoutItem<'_, ()>) -> bool {
    is_autospace_inline_box_for_brush(item)
}

fn is_autospace_inline_box_for_brush<B: parley::Brush>(item: &PositionedLayoutItem<'_, B>) -> bool {
    matches!(
        item,
...
fn hanging_baseline_delta<B: parley::Brush>(
...
pub(crate) fn draw_text_node(
    ...
    if let Some(text_layout) = node.shared_inline_text_layout() {
        draw_text_layout(
            scene,
            node,
            cascade,
...
trait TextBrushOwner: parley::Brush {
    fn belongs_to_node(&self, node_id: usize) -> bool;
}

impl TextBrushOwner for () {
    fn belongs_to_node(&self, _node_id: usize) -> bool {
        true
    }
}

impl TextBrushOwner for usize {
    fn belongs_to_node(&self, node_id: usize) -> bool {
        *self == node_id
    }
}

fn draw_text_layout<B: TextBrushOwner>(
    scene: &mut impl PaintScene,
    node: &Node,
    cascade: &CascadeResult,
    node_id: usize,
    text_layout: &parley::Layout<B>,
    position: TextPosition,
    decorations: &DecorationContext,
) {
    let TextPosition {
    ...
    for (line_start, line_end, fragment_x, fragment_y) in fragments {
        ...
        for (line_index, line) in text_layout.lines().enumerate() {
            ...
            let line_has_autospace = line
                .items()
                .any(|item| is_autospace_inline_box_for_brush(&item));
            ...
            for item in line.items() {
                ...
                if !glyph_run.style().brush.belongs_to_node(node_id) {
                    continue;
                }
...
fn leading_whitespace_advance<B: parley::Brush>(line: &parley::Line<'_, B>, rtl: bool) -> f32 {
