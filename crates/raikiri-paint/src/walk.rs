    ...
    while let Some(frame) = stack.pop() {
        ...
        match node.kind() {
            NodeKind::Element => {
                ...
                let shared_fragment_intersects_page =
                    node.shared_inline_fragments().is_some_and(|fragments| {
                ...
                let mut paints_on_page = !visibility_hidden_table
                    && (node_id == body_id
                        || ((fixed_in_viewport || named_page_matches(node_id))
                            && (fixed_in_viewport
                                || inside_fixed
                                || shared_fragment_intersects_page
                                || box_intersects_page(
                                    abs_y,
                                    layout.size.height,
                                    page_top,
                ...
                if paints_on_page {
                    ...
                    if node_id != body_id {
                        ...
                        if let Some(fragments) = node.shared_inline_fragments() {
            ...
            NodeKind::Text => {
                ...
                let abs_x = parent_abs_x + layout.location.x;
                let abs_y = parent_abs_y + layout.location.y;
                let paint_text_height = node
                    .shared_inline_text_layout()
                    .map(|layout| layout.height())
                    .unwrap_or(layout.size.height);
                if named_page_matches(node_id)
                    && (inside_fixed
                        || box_intersects_page(abs_y, paint_text_height, page_top, page_bottom))
                {
