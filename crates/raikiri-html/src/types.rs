...
/// (M1) に加えて、M2 (raikiri-spike-5z86.6) から `<head>` 内
/// `<link rel="stylesheet">` を `ParseOptions::network` 経由で fetch した CSS
/// text も Author stylesheet として集約される (`parse.rs::parse_with_sink` が
...
/// (`StylesheetKind::Author`) 経由で消費する。`network` / `base_url` は M2
/// (raikiri-spike-5z86.6) から `parse_with_sink` が `<head>` 内
...
pub struct ParseOptions<'a> {
    ...
    /// fetch されず無視される (raikiri-spike-5z86.6、外部 stylesheet 機能は
    /// opt-in)。Replaced element の fetch はこの provider を受け取るのみで
    /// まだ未消費 (scope 外)。
