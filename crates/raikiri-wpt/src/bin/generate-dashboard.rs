...
fn main() {
    ...
    let output = args
        ...
        .unwrap_or_else(|| PathBuf::from("docs/wpt-dashboard.html"));
    ...
    println!(
        "Dashboard is static at docs/wpt-dashboard.html. Run `cargo test -p raikiri-wpt --test basic_reftests` to update data, then re-run this bin when dynamic generation is implemented."
    );
    println!("Output requested: {}", output.display());
    // Ensure the static file exists
    if !output.exists() {
        eprintln!(
            "Warning: {} does not exist. Copying from docs/wpt-dashboard.html if available.",
            output.display()
        );
    }
}
