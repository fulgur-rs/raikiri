...
#[test]
fn bin_compare_reports_improvements_and_exits_zero_without_regressions() {
    let wpt_root = tempfile::tempdir().unwrap();
    assert_clean_page(wpt_root.path(), "pass.html");
    write_page(
        wpt_root.path(),
        "root-parent.html",
        &page(
            "",
            "test(function() { assert_true(document.documentElement.parentNode === document); }, 'root parent');",
        ),
    );

    let assertion = Command::cargo_bin("run-css-text-i18n")
        .unwrap()
        .args(["--compare", "--wpt-root", wpt_root.path().to_str().unwrap()])
        .assert()
        .code(0);
    let stdout = String::from_utf8(assertion.get_output().stdout.clone()).unwrap();
    assert!(!stdout.contains("REGRESSION"), "{stdout}");
    assert!(
        stdout.contains("IMPROVED css/css-text/i18n/root-parent.html: legacy "),
        "{stdout}"
    );
    assert!(stdout.contains(", native 1/1"), "{stdout}");
    assert!(
        stdout.contains("-- compare: 0 regressions, 1 improvements, 2 files;"),
        "{stdout}"
    );
}

#[test]
fn bin_compare_lists_regressions_and_exits_one() {
    let wpt_root = tempfile::tempdir().unwrap();
    write_page(
        wpt_root.path(),
        "root-parent-null.html",
        &page(
            "",
            "test(function() { assert_true(document.body.parentNode.parentNode === null); }, 'legacy root parent');",
        ),
    );

    let assertion = Command::cargo_bin("run-css-text-i18n")
        .unwrap()
        .args(["--wpt-root", wpt_root.path().to_str().unwrap(), "--compare"])
        .assert()
        .code(1);
    let stdout = String::from_utf8(assertion.get_output().stdout.clone()).unwrap();
    assert!(
        stdout
            .contains("REGRESSION css/css-text/i18n/root-parent-null.html: legacy 1/1, native 0/1"),
        "{stdout}"
    );
    assert!(
        stdout.contains("-- compare: 1 regressions, 0 improvements, 1 files;"),
        "{stdout}"
    );
}

#[test]
fn bin_compare_reports_a_missing_wpt_root_as_a_run_error() {
    let missing = TempDir::new().unwrap().path().join("not-present");
    let assertion = Command::cargo_bin("run-css-text-i18n")
        .unwrap()
        .args(["--compare", "--wpt-root", missing.to_str().unwrap()])
        .assert()
        .code(2);
    let stderr = String::from_utf8(assertion.get_output().stderr.clone()).unwrap();
    assert!(
