...
fn line_break_anywhere_supported_registry_subset_is_pixel_exact_at_800x600() {
    ...
    // Initial registry-only support matrix: 19 exact static cases. Deferred:
    // `line-break-anywhere-005.html` and `line-break-anywhere-009.html` need
    // `break-spaces`; `line-break-anywhere-008.html` needs preserved-space
    // hanging controls. The unprefixed `line-break-anywhere-006.html` and
    // `line-break-anywhere-010.html` need GL/NBSP overrides (`-010` also uses
    // `pre-wrap`). The included `line-break-anywhere-overrides-uax-behavior-*`
    // files are separate cases. `line-break-anywhere-003.html` needs JavaScript.
...
fn line_break_anywhere_preserves_mandatory_newlines_and_pre_wrap_fallback() {
    ...
    // Registry Parley has no CSS hanging-space controls. Keep pre-wrap on its
    // existing path and compare preserved-space runs directly with `normal`.
    std::fs::write(
        &test,
