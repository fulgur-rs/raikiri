...
/// comparable in scope to the full Unicode Bidirectional Algorithm's
/// paragraph-embedding-level and weak/neutral-type resolution rules that
/// [`strong_bidi_type`]'s doc similarly declines to implement for `:dir()`'s
/// `auto` value — this function skips both and operates directly on the raw
/// hyphen-split subtag strings as written. In practice this only
/// under-rejects (a
