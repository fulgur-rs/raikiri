...
/// なく `-50%` / `-1em` も drop する。`<absolute-size>` / `<relative-size>` は
/// grammar 上そもそも符号を持たないので本 constraint の対象外。
fn parse_font_size(input: &mut Parser<'_, '_>) -> Option<PropertyValue> {
    ...
    (v >= 0.0).then_some(PropertyValue::FontSize(length))
}
...
/// 抽出して `>= 0.0` を post-filter する。`parse_font_size` は
/// raikiri-spike-4rmu で `<absolute-size>` / `<relative-size>` / `math` の
/// ident 分岐 (`parse_font_size_keyword`) が前段に付いたため関数全体としては
/// 同形ではなくなったが、この tail 部分のロジックは identical。
///
